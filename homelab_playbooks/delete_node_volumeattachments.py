    ...
    for volume_attachment in volume_attachments.items:
        if volume_attachment.spec.node_name == node.metadata.name:
            try:
                ...
                raise ActionException(ErrorCodes.ACTION_UNEXPECTED_ERROR, f"delete_node_volumeattachments: failed to delete node volumeattachment {volume_attachment.metadata.name}: {e}")
